...
class Zhihu():
    ...
    def cap_again(self,driver,num):
        num = str(num)
        self.sleep(3)
        if "我们检测到您此次登录异常，请将 App 升级至新版本，按照提示进行安全验证。" in driver.page_source:
            """
    ...
    def guanzhu(self,driver,url):
        ...
        if '已关注' in driver.page_source:
            return driver
        guanzhu_but = driver.find_element_by_xpath('//*[@id="ProfileHeader"]/div/div[2]/div/div[2]/div[3]/div/button[1]')
        guanzhu_but.click()
        return driver
        ...
        if os.path.exists(os.path.join(os.path.join(os.getcwd(),'cookie',str(num)+'.json'))):
            ...
            if '手机验证码登录' in driver.page_source or '注册即代表你同意' in driver.page_source:
                ...
                if type(cookie) == int:
                    ...
                    driver = self.load_cookie(driver=driver,num=num) #如果这里能正常，就能回到主页！
    ...
    def set_headimg(self,driver):
        ...
        self.sleep(3)
        head_elem = driver.find_element_by_xpath('//*[@id="root"]/div/main/div/div/div[2]/div[1]/input')
    ...
    def zan_ci(self,driver,url): #点赞完成后，计算这个bot对户主点赞了多少次！
        driver.get(url)
        elem = driver.find_element_by_class_name('FollowshipCard-link')
        text = re.findall("[0-9]{1,5}",elem.text)
        return text[0]
...
if __name__ == '__main__':
    a = Zhihu()
    driver = webdriver.Chrome()
    driver = a.load_cookie(num=15584164978,driver=driver) #返回已经登陆的页面
    print(a.zan_ci(driver=driver,url='https://www.zhihu.com/people/kong-41-52/'))
